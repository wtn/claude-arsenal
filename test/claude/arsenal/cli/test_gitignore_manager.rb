  ...
  module Arsenal
    module CLI
      class TestGitignoreManager < Minitest::Test
        ...
        def test_adds_entries_for_solo_mode
          manager = GitignoreManager.new

          capture_io do
            manager.add_entries(mode: :solo)
          end
          ...
          assert_match(/^\/\.claude\/$/, content)
          assert_match(/^\/dev\/$/, content)
        end

        def test_adds_entries_for_team_mode
          manager = GitignoreManager.new

          capture_io do
            manager.add_entries(mode: :team)
          end
          ...
          refute_match(/^\/\.claude\/$/, content)  # Should not have bare /.claude/
        end
        ...
        def test_updates_existing_section
          ...
          # Should have team mode entries with leading slashes
          assert_match(/^\/\.claude\/tmp\/$/, content)
          assert_match(/^\/dev\/active\/$/, content)
